        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-2">
                <div class="flex justify-end p-2">
                    @can('User create')
                        <a href="{{ route('admin.users.create') }}" class="px-4 py-2 bg-green-700 hover:bg-green-500 text-slate-100 rounded-md">Create User</a>
                    @endcan
                </div>
                                        ...
                                        <tr class="border-b dark:border-neutral-500">
                                            ...
                                            <td class="whitespace-nowrap  px-6 py-4">
                                                <div class="flex space-x-2">
                                                    @can('User edit')
                                                        <a href="{{ route('admin.users.edit', $user->id) }}" class="px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white rounded-md">Edit</a>
                                                    @endcan

                                                    @can('User delete')
                                                        <form method="POST" action="{{ route('admin.users.destroy', $user->id) }}" onsubmit="return confirm('Are you sure want to delete?');">
                                                            @csrf
                                                            @method('DELETE')
